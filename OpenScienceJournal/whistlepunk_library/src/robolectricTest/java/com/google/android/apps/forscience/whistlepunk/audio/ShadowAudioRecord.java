...
public class ShadowAudioRecord {
    ...
    @Implementation
    protected void __constructor__(int audioSource, int sampleRateInHz, int channelConfig,
    ...
    protected int read(short[] audioData, int offsetInShorts, int sizeInShorts) {
        synchronized (mAudioDataLock) {
            if (mAudioData.size() > 0) {
                System.arraycopy(Shorts.toArray(mAudioData.subList(0,
                        sizeInShorts > mAudioData.size() ? mAudioData.size() : sizeInShorts)), 0,
