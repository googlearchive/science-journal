...
public class ExpandableDeviceAdapterTest {
  ...
  private DataController mDataController =
      new InMemorySensorDatabase().makeSimpleController(mMetadataManager);
  ...
  private final TestDevicesPresenter mPresenter =
      new TestDevicesPresenter(mAvailableDevices, mPairedDevices);
  ...
  private class Scenario {
    ...
    private ConnectableSensor makeConnectedSensorNewDevice() {
      return mConnector.connected(makeSensorNewDevice().asGoosciSpec(), newConnectedSensorId());
    }
  ...
  public void devicesAtTopLevel() {
    ExpandableDeviceAdapter adapter =
        ExpandableDeviceAdapter.createEmpty(mSensorRegistry, mDeviceRegistry, null, null, 0);
    ...
    assertEquals(
        mDeviceRegistry.getDevice(sensor.getSpec()).getName(),
  ...
  public void replaceOnSecondAddSameKey() {
    ...
    adapter.addSensor(
        key, mConnector.connected(replacement.asGoosciSpec(), sensor1.getConnectedSensorId()));
    ...
    adapter.addSensor(
        newKey, mConnector.connected(newReplace.asGoosciSpec(), newSensor.getConnectedSensorId()));
  ...
  public void checkBuiltInWhenForgettingLastSensor() {
    ...
    TestDevicesPresenter presenter = new TestDevicesPresenter(mAvailableDevices, null);
    ConnectableSensorRegistry sensorRegistry =
        new ConnectableSensorRegistry(
            ...
            mConnector);
    ExpandableDeviceAdapter adapter =
        ExpandableDeviceAdapter.createEmpty(sensorRegistry, mDeviceRegistry, null, mSensors, 0);
    ...
    ExperimentSensors sensors =
        mMetadataManager.getExperimentSensors(
            experiment.getExperimentId(), Maps.<String, SensorProvider>newHashMap(), mConnector);
