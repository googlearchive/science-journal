...
import static com.google.android.apps.forscience.whistlepunk.AudioSource.SAMPLE_RATE_IN_HZ;
...
/**
 * An experimental sensor that displays the pitch in Hertz (Hz).
 */
public class ExperimentalPitchSensor extends ScalarSensor {
    public static final String ID = "ExperimentalPitchSensor";
            ...
            public void onReceiveAudio(short[] audioSourceBuffer) {
                ...
                while (audioSourceBufferOffset < audioSourceBuffer.length) {
                    ...
                    if (audioAnalyzerBufferOffset == audioAnalyzerBuffer.length) {
                        ...
                        if (frequency != null && mPreviousFrequency != null) {
                            ...
                            if (Math.abs(frequency - mPreviousFrequency) / mPreviousFrequency > 0.50) {
                                frequency = null;
                            }
                        }
                        if (frequency != null) {
                            c.addData(clock.getNow(), frequency);
                        }
