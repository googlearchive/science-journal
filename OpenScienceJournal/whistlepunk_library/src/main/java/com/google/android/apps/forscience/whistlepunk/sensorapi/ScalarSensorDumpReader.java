...
public class ScalarSensorDumpReader {
    ...
    public void readData(GoosciScalarSensorData.ScalarSensorData scalarSensorData, Map<String,
            String> idMap) {
        ...
        for (GoosciScalarSensorData.ScalarSensorDataDump sensor : scalarSensorData.sensors) {
           ...
           try (BatchDataController batchController =
                         new BatchDataController(mDataController)) {
    ...
    private void addAllRows(GoosciScalarSensorData.ScalarSensorDataDump sensor,
        ...
        for (GoosciScalarSensorData.ScalarSensorDataRow row : sensor.rows) {
            addData(batchController, zoomRecorder, trialId, sensor.tag, row.timestampMillis,
                    row.value);
        }
        zoomRecorder.flushAllTiers(batchController);
    }

    private boolean addData(RecordingDataController dataController, ZoomRecorder zoomRecorder,
            String trialId, String tag,
            final long timestampMillis, double value) {
    ...
    private void recordData(RecordingDataController batchController, ZoomRecorder zoomRecorder,
            String trialId, String tag,
