...
public class FrequencyOptionsPresenter implements SensorPresenter.OptionsPresenter {
    ...
    public FrequencyOptionsPresenter(FilterChangeListener filterChangeListener,
    ...
    public View buildOptionsView(final ActiveBundle activeBundle, Context context) {
        @SuppressLint("InflateParams") final View inflated =
                LayoutInflater.from(context).inflate(R.layout.frequency_options, null);

        if (mAdditionalPresenter != null) {
            final ViewGroup additionalView =
                    (ViewGroup) inflated.findViewById(R.id.additional_options);
        ...
        windowEditText.addTextChangedListener(
                new LongUpdatingWatcher(activeBundle, PREFS_KEY_FREQUENCY_WINDOW,
                        windowEditText));
        ...
        filterEditText.addTextChangedListener(
                new FloatUpdatingWatcher(activeBundle, PREFS_KEY_FREQUENCY_FILTER,
                        filterEditText));

        CheckBox enableFrequencyBox = getFrequencyCheckbox(inflated);
        enableFrequencyBox.setChecked(getFrequencyChecked(roBundle));
        enableFrequencyBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    @Override
    ...
    public void applyOptions(ReadableSensorOptions prefs) {
        ...
        mFilterChangeListener.setScalarFilter(
                ScalarSensor.computeValueFilter(newWindow, newFilter, newEnabled,
