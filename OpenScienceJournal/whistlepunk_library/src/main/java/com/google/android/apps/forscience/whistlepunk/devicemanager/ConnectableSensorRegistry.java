...
public class ConnectableSensorRegistry {
  ...
  private final Map<String, SensorDiscoverer.SettingsInterface> mSettingsIntents = new ArrayMap<>();
                ...
                new LoggingConsumer<Success>(TAG, "Load appearance") {
                  ...
                  public void success(Success value) {
                    if (sensor.shouldShowOptionsOnConnect() && settings != null) {
                      mPresenter.showSensorOptions(
                          mExperimentId, sensor.getConnectedSensorId(), settings);
                    }
                  }
                });
          }
  ...
  private void onMyDevicesLoaded(
    ...
    mDataController.getExternalSensorsByExperiment(
        mExperimentId,
        new LoggingConsumer<ExperimentSensors>(TAG, "Load external sensors") {
          ...
          private void addBuiltInSensors(
            ...
            for (String sensorId : sr.getBuiltInSources()) {
              allSensors.add(
                  mConnector.builtIn(sensorId, !sensors.getExcludedSensorIds().contains(sensorId)));
            }
          }
      ...
      DeviceOptionsViewController.maybeReplaceSensor(
          ...
          new LoggingConsumer<String>(TAG, "replacing sensor on scan") {
            ...
            public void success(String newSensorId) {
              mOptionsListener.onExperimentSensorReplaced(oldSensorId, newSensorId);
              mSensors.put(sensorKey, mConnector.connected(newSensor.getSensorSpec(), newSensorId));
            }
          });
    }
  }
  ...
  public void addSensorIfNecessary(
    ...
    if (spec != null) {
      ...
      mDataController.addOrGetExternalSensor(
          pairedSpec,
          MaybeConsumers.chainFailure(
              ...
              new Consumer<String>() {
                ...
                public void take(final String sensorId) {
                  addSensorToCurrentExperiment(
                      mConnector.connected(pairedSpec.asGoosciSpec(), sensorId), onAdded);
                }
              }));
    } else {
  ...
  private void addSensorToCurrentExperiment(
    ...
    mDataController.addSensorToExperiment(
        mExperimentId,
  ...
  public void forgetMyDevice(
    ...
    removeSensorsFromExperiment(
        ...
        new LoggingConsumer<Success>(TAG, "removing sensors") {
          @Override
          public void success(Success value) {
            mDataController.forgetMyDevice(
                spec,
  ...
  public void reloadProvider(String providerKey, boolean startSpinners) {
    ...
    if (discoverer == null) {
      throw new IllegalArgumentException("Couldn't find " + providerKey + " in " + mDiscoverers);
    }
    startScanning(
        providerKey,
        discoverer,
