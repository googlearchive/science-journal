...
public class AudioAnalyzer {
    ...
    private static final Comparator<Peak> FREQUENCY_ASCENDING = (first, second) ->
            (int) Math.signum(first.getFrequency() - second.getFrequency());
    ...
    public Double detectFundamentalFrequency(short[] samples) {
        ...
        double uncalibratedDecibels = SoundUtils.calculateUncalibratedDecibels(
        ...
        for (Peak peak : mPeaks) {
            double frequency = mGoertzelAnalyzer.findFrequencyWithHighestPower(samples,
                    peak.getFrequencyEstimate());
    ...
    /**
     * Determines the fundamental frequency by examining the harmonic ratios between peaks in
     * the FFT output. Returns null if no harmonic ratios are identified.
     */
    ...
    /**
     * Identifies the harmonic ratio between the two given peaks. Returns null if no harmonic
     * ratio is found.
     */
