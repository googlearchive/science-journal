...
public class ChartView extends View {
  ...
  public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    mRightPadding = getResources().getDimensionPixelSize(R.dimen.stream_presenter_padding_sides);
  ...
  private void measure() {
    ...
    mPathPaint.setPathEffect(
        new CornerPathEffect(res.getDimensionPixelSize(mChartOptions.getCornerPathRadiusId())));
    mPathPaint.setStrokeWidth(res.getDimensionPixelSize(mChartOptions.getLineWidthId()));
    mAxisPaint.setStrokeWidth(res.getDimensionPixelSize(mChartOptions.getAxisLabelsLineWidthId()));
    ...
    mAxisTextStartPadding = res.getDimensionPixelSize(mChartOptions.getAxisLabelsStartPaddingId());
    ...
    mRecordingTimePaint.setStrokeWidth(
    ...
    makeDashedLinePaint(mLabelLinePaint, R.color.note_overlay_line_color, mStatLineWidth, dashSize);
    ...
    mChartRect =
        new RectF(
            mStartPadding, mTopPadding, mChartWidth + mStartPadding, mChartHeight + mTopPadding);
  }
  ...
  public void initialize(ChartOptions chartOptions, ChartData chartData) {
    ...
    final boolean isObserve =
        mChartOptions.getChartPlacementType() == ChartOptions.ChartPlacementType.TYPE_OBSERVE;
            ...
            public boolean onTouch(View v, MotionEvent event) {
              ...
              if (type == MotionEvent.ACTION_DOWN) {
                ...
                if (mChartOptions.canPanX()) {
                  ...
                  mChartOptions.setRenderedXRange(
                      mChartOptions.getRenderedXMin() + dPanX,
                ...
                if (numPointers > 1) {
                  ...
                  if (mChartOptions.canZoomX() && xZoomSpan > touchSlop) {
                    ...
                    long newDiff =
                        (long)
                            ((mChartOptions.getRenderedXMax() - mChartOptions.getRenderedXMin())
                                * scale);
                    long avg =
                        (mChartOptions.getRenderedXMax() + mChartOptions.getRenderedXMin()) / 2;

                    mChartOptions.setRenderedXRange(avg - newDiff / 2, avg + newDiff / 2);
                  ...
                  if (mChartOptions.canZoomY() && yZoomSpan > ZOOM_SLOP) {
                    ...
                    double newDiff =
                        Math.min(
                                mChartOptions.getMaxRenderedYRange(),
                                mChartOptions.getRenderedYMax() - mChartOptions.getRenderedYMin())
                            * scale;
                    double avg =
                        (mChartOptions.getRenderedYMax() + mChartOptions.getRenderedYMin()) / 2;
                    double newMin =
                        isObserve
                            ? avg - newDiff / 2
                ...
                boolean eventUsed =
                    (hasPannedX && significantPan(mTouchX - oldTouchX))
                        || hasPannedY
                        || hasZoomedX
                        || hasZoomedY;
                if (eventUsed) {
  ...
  public double getYFromScreenY(float y) {
    return mChartOptions.getRenderedYMin()
        + getYValueDeltaFromYScreenDelta(mHeight - mBottomPadding - y);
  }
  ...
  private void populatePath(boolean optimizePinnedToEnd) {
    ...
    if (optimizePinnedToEnd) {
      ...
      points =
          mChartData.getPointsInRange(
              mChartOptions.getRenderedXMin() - BUFFER_MS,
    ...
    if (!mHasPath
      ...
      if (mChartOptions.isPinnedToNow() && !mWasPinnedToNow) {
        ...
        postInvalidateOnAnimation();
          ...
          || mChartOptions.getRenderedXMax() >= point.getX()
          || mLeadingEdgeIsDrawn) {
  ...
  public void transformPath() {
    ...
    mMatrix.reset();
    mPreviousChartRect.set(
        getScreenX(mXMinForPathCalcs),
        getScreenY(mYMaxForPathCalcs),
        getScreenX(mXMaxForPathCalcs),
        getScreenY(mYMinForPathCalcs));
  ...
  private float getPathX(long x) {
    return (1.0f * (x - mXMinForPathCalcs)) / (mXMaxForPathCalcs - mXMinForPathCalcs) * mChartWidth
        + mStartPadding;
  }
  ...
  private float getPathY(double y) {
    return (float)
        (mChartHeight * (1 - ((y - mYMinForPathCalcs) / (mYMaxForPathCalcs - mYMinForPathCalcs)))
            + mTopPadding);
  }
  ...
  public void onDraw(Canvas canvas) {
    ...
    canvas.drawRect(mWidth - mRightPadding, 0, mWidth, mHeight, mBackgroundPaint);
  ...
  private void drawTriggers(Canvas canvas) {
    ...
    for (double value : triggerValues) {
      ...
      drawable.setBounds(
          (int) (mStartPadding - width - mAxisTextStartPadding),
          (int) y - height / 2,
          (int) (mStartPadding - mAxisTextStartPadding),
          (int) y + height / 2);
  ...
  private void drawYAxisText(Canvas canvas) {
    ...
    for (int i = 0; i < mYAxisPoints.size(); i++) {
      ...
      canvas.drawText(
          mYAxisPointLabels.get(i),
          mStartPadding - mAxisTextStartPadding,
          yValue + mAxisTextHeight / 3,
          mAxisTextPaint);
    }
  }
  ...
  private void updateYAxisPoints() {
    ...
    if (sizeShown < MINIMUM_NUM_LABELS || sizeShown > MAXIMUM_NUM_LABELS) {
      ...
      while (nextSmallerLabel > yMin) {
        ...
        mYAxisPointLabels.add(0, mChartOptions.getAxisNumberFormat().format(nextSmallerLabel));
  ...
  public void onAxisLimitsAdjusted() {
    ...
    boolean newRangeOutsideOfPathRange =
        (mChartOptions.getRenderedXMax() > mXMaxInPath && mXMaxInPath < mChartData.getXMax())
