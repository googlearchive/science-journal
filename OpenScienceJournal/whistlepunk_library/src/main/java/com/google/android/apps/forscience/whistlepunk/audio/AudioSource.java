...
public class AudioSource {
    ...
    private final int mMinBufferSizeInBytes;
    private final Object mLockAudioReceivers = new Object();
    private final List<AudioReceiver> mAudioReceivers = new ArrayList<>();

    public interface AudioReceiver {
        /**
         * Called when a full buffer of audio data has been read and is ready to be processed by
         * the AudioReceiver.
         */
    ...
    public void unregisterAudioReceiver(AudioReceiver audioReceiver) {
        ...
        synchronized (mLockAudioReceivers) {
            if (!mAudioReceivers.isEmpty()) {
                mAudioReceivers.remove(audioReceiver);
                needToStop = mAudioReceivers.isEmpty();
            }
        }
        if (needToStop) {
            stop();
    ...
    private void start() {
        // FYI: the current thread holds mLockAudioReceivers.
        // Use VOICE_COMMUNICATION to filter out audio coming from the speakers
        final AudioRecord audioRecord = new AudioRecord(
                MediaRecorder.AudioSource.VOICE_COMMUNICATION, SAMPLE_RATE_IN_HZ, CHANNEL_CONFIG,
                AUDIO_FORMAT, mMinBufferSizeInBytes);
        ...
        mFuture = mExecutorService.submit(() -> {
            short[] buffer = new short[mMinBufferSizeInBytes / 2];
            int offset = 0;
            ...
            while (mRunning.get()) {
                ...
                if (!goodDataRead) {
                    ...
                    if (countLeadingZeros > 0) {
                        if (readShorts > countLeadingZeros) {
                            System.arraycopy(buffer, countLeadingZeros,
                                    buffer, 0, readShorts - countLeadingZeros);
                        }
                ...
                if (offset == buffer.length) {
                    synchronized (mLockAudioReceivers) {
                        for (AudioReceiver audioReceiver : mAudioReceivers) {
    ...
    public void unregisterAllAudioReceivers() {
        synchronized (mLockAudioReceivers) {
            while (!mAudioReceivers.isEmpty()) {
                unregisterAudioReceiver(mAudioReceivers.get(0));
            }
        }
    }
}
