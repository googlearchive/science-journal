...
public abstract class ExternalAxisView extends View {
  ...
  protected void drawTicks(
    ...
    if (!labelTick) {
      drawLabel(
          firstTickTime - mTimeBetweenTicks,
    ...
    for (long t = firstTickTime; t <= lastTickTime; t += mTimeBetweenTicks) {
      if (labelTick) {
        canvas.drawLine(xOffset, tickPaddingTop, xOffset, tickPaddingTop + mLongTickHeight, mPaint);
        drawLabel(t, xOffset, tickPaddingTop, canvas);
      } else {
        canvas.drawLine(
            xOffset, tickPaddingTop, xOffset, tickPaddingTop + mShortTickHeight, mPaint);
      }
  ...
  private void drawLabel(long t, float xOffset, float tickPaddingTop, Canvas canvas) {
    ...
    canvas.drawText(
        ...
        (tickPaddingTop + mTextHeight + mTickPaddingBottom + mLongTickHeight),
        mTextPaint);
  }
